...
export async function importAppConfig(file: File) {
    ...
    useConfigStore.setState({ config: { ...defaultConfig, ...data.config, reversePrompt: typeof data.config.reversePrompt === "string" ? data.config.reversePrompt : defaultConfig.reversePrompt }, webdav: data.webdav });
