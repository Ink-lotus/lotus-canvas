            ...
            {inputs.map((input, index) => (
                ...
                >
                    ...
                    <span className="min-w-0 flex-1">
                        <span className="block truncate font-medium">{input.title || resourceLabel(input, allInputs)}</span>
                        <span className="block truncate opacity-65">{input.type === "group" ? i18n.t("canvas.node.nodeCount", { count: input.children.length }) : input.text || resourceLabel(input, allInputs)}</span>
                    </span>
                </button>
            ))}
        </div>
...
function createReferenceChip(input: NodeGenerationInput, inputs: NodeGenerationInput[], theme: (typeof canvasThemes)[keyof typeof canvasThemes], onImagePreview: (url: string) => void) {
    ...
    if (input.type === "image" && input.image) {
        ...
        text.textContent = input.type === "text" ? input.title || resourceLabel(input, inputs) : input.title;
