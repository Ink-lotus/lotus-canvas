export default {
    ...
    config: {
        ...
        localStorage: {
            ...
            migration: {
                legacy: "待迁移媒体",
                pending: "{{count}} 个 IndexedDB 文件",
                done: "迁移已完成",
                action: "迁移现有媒体到桌面文件库",
                confirmTitle: "迁移现有媒体？",
                confirmDescription: "将 {{count}} 个文件（{{size}}）逐个迁移到 {{path}}。每个文件校验成功后才会删除 IndexedDB 原件。",
                start: "开始迁移",
                cancel: "完成当前文件后停止",
                progress: "正在迁移 {{current}} / {{total}}",
                summary: "已迁移 {{migrated}} 个，失败 {{failed}} 个",
                completed: "已迁移 {{count}} 个媒体文件",
                partial: "迁移完成：{{migrated}} 个成功，{{failed}} 个失败",
                canceled: "迁移已停止，可以稍后继续",
                failed: "迁移失败",
                noSpace: "磁盘可用空间不足以迁移最大的单个文件",
            },
        ...
        channels: {
            description: "每个渠道选择一个协议并拉取模型，为每个模型指定能力、统一模型名（可选）并可自定义调用脚本。",
            add: "新增渠道",
            unnamed: "未命名渠道",
        ...
        preferences: {
            ...
            reversePrompt: "默认反推提示词",
            reversePromptDescription: "点击图片节点的「反推提示词」时，将使用这段文字创建任务说明文本节点。",
            reversePromptPlaceholder: "输入用于图片反推的默认提示词。",
            systemPrompt: "系统提示词",
        ...
        channelEditor: {
            ...
            maxConcurrencyDescription: "该渠道的图片任务共享此并发上限，范围 1–20。",
            models: "渠道模型",
            modelDescription: "已选 {{count}} 个；可为每个模型设置统一名称、能力和调用脚本。",
            alias: "统一模型名",
            aliasPlaceholder: "可选；用于跨渠道归组",
            selectModels: "选择模型",
            scriptReady: "脚本已设",
            script: "调用脚本",
