...
function InfiniteCanvasPage() {
            ...
            prev.map((node) => {
                ...
                return {
                    ...
                    metadata: {
                        ...
                        model: image.model || node.metadata?.model,
    ...
    const cropImageNode = useCallback(async (node: CanvasNodeData, crop: CanvasImageCropRect) => {
        ...
        const image = await uploadImage(cropped, { origin: "generated" });
        ...
        async (node: CanvasNodeData, params: CanvasImageSplitParams) => {
            ...
            const childNodes = await Promise.all(
                pieces.map(async (piece) => {
                    const image = await uploadImage(piece.dataUrl, { origin: "generated" });
                    const id = nanoid();
                    return {
                        id,
                        type: CanvasNodeType.Image,
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            try {
                ...
                // The mask stays inside the run closure so every channel attempt sends it; a model that rejects masks falls back to the next channel.
                const scheduled = await scheduleImageGeneration(
                    generationConfig,
                    generationConfig.imageModelTargets,
                    (target) => requestEdit({ ...generationConfig, model: target, imageModel: target }, prompt, [source], mask, { signal: controller.signal }),
                    { signal: controller.signal, preferredTarget: generationConfig.imageModel, fallbackOnError: true },
                );
                const image = scheduled.value[0];
                const uploaded = await uploadImage(image.dataUrl, { origin: "generated" });
                const size = fitNodeSize(uploaded.width, uploaded.height, node.width, node.height);
                setNodes((prev) => prev.map((item) => (item.id === childId ? { ...item, width: size.width, height: size.height, metadata: { ...item.metadata, ...imageMetadata(uploaded), prompt, ...generationMetadata, model: scheduled.target } } : item)));
            } catch (error) {
    ...
    const upscaleImageNode = useCallback(async (node: CanvasNodeData, params: CanvasImageUpscaleParams) => {
        ...
        const image = await uploadImage(upscaled, { origin: "generated" });
        const size = fitNodeSize(image.width, image.height);
        const childId = nanoid();
        const child: CanvasNodeData = {
            id: childId,
        ...
        async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
            ...
            try {
                const scheduled = await scheduleImageGeneration(
                    generationConfig,
                    generationConfig.imageModelTargets,
                    (target) => requestEdit({ ...generationConfig, model: target, imageModel: target }, prompt, [source], undefined, { signal: controller.signal }),
                    { signal: controller.signal, preferredTarget: generationConfig.imageModel, fallbackOnError: true },
                );
                const image = scheduled.value[0];
                const uploaded = await uploadImage(image.dataUrl, { origin: "generated" });
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                try {
                    ...
                    const scheduled = await scheduleImageGeneration(
                        ...
                        generationConfig.imageModelTargets,
                        (target) => {
                            const requestConfig = { ...generationConfig, model: target, imageModel: target, count: "1" };
                            return refs.length ? requestEdit(requestConfig, context.prompt, refs, undefined, { signal: controller.signal }) : requestGeneration(requestConfig, context.prompt, { signal: controller.signal });
                        },
                        { signal: controller.signal, preferredTarget: generationConfig.imageModel, fallbackOnError: true },
                    );
                    const image = scheduled.value[0];
                    const uploaded = await uploadImage(image.dataUrl, { origin: "generated" });
                    setNodes((prev) =>
                        prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, ...imageMetadata(uploaded), prompt: scene, model: scheduled.target, imageModelTargets: generationConfig.imageModelTargets, status: NODE_STATUS_SUCCESS, errorDetails: undefined } } : node)),
                    );
                    setDialogNodeId(null);
                } catch (error) {
                        ...
                        imageIds.map(async (imageId, index) => {
                            try {
                                const scheduled = await scheduleImageGeneration(
                                    ...
                                    (target) => {
                                        const requestConfig = { ...generationConfig, model: target, imageModel: target, count: "1" };
                                        return referenceImages.length ? requestEdit(requestConfig, effectivePrompt, referenceImages, undefined, { signal: controller.signal }) : requestGeneration(requestConfig, effectivePrompt, { signal: controller.signal });
                                    },
                                    { signal: controller.signal, preferredTarget: index === 0 ? generationConfig.imageModel : undefined, fallbackOnError: count === 1 },
                                );
                                const image = scheduled.value[0];
                                const uploaded = await uploadImage(image.dataUrl, { origin: "generated" });
                                const imageSize = fitNodeSize(uploaded.width, uploaded.height, imageConfig.width, imageConfig.height);
                                const item: CanvasNodeImage = { id: imageId, status: NODE_STATUS_SUCCESS, content: uploaded.url, storageKey: uploaded.storageKey, naturalWidth: uploaded.width, naturalHeight: uploaded.height, bytes: uploaded.bytes, mimeType: uploaded.mimeType, model: scheduled.target };
                                setNodes((prev) =>
                                    prev.map((node) => {
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            const generationConfig =
                hasSavedImageMetadata && savedImageMetadata
                    ? {
                          ...
                          imageModelTargets: savedImageMetadata.imageModelTargets?.length ? savedImageMetadata.imageModelTargets : [savedImageMetadata.model || effectiveConfig.imageModel || effectiveConfig.model],
            ...
            if (!isAiConfigReady(generationConfig, generationConfig.model) || (node.type === CanvasNodeType.Image && generationConfig.imageModelTargets.some((target) => !isAiConfigReady(generationConfig, target)))) {
                openConfigDialog(true);
                return;
            }
            ...
            try {
                ...
                const scheduled = await scheduleImageGeneration(
                    ...
                    generationConfig.imageModelTargets,
                    (target) => {
                        const requestConfig = { ...generationConfig, model: target, imageModel: target };
                        return useReferenceImages ? requestEdit(requestConfig, prompt, retryImages, undefined, { signal: controller.signal }) : requestGeneration(requestConfig, prompt, { signal: controller.signal });
                    },
                    {
                        signal: controller.signal,
                        preferredTarget: (node.metadata?.images?.length || 1) === 1 ? generationConfig.imageModel : undefined,
                        fallbackOnError: (node.metadata?.images?.length || 1) === 1,
                    },
                );
                const image = scheduled.value[0];
                const uploadedImage = await uploadImage(image.dataUrl, { origin: "generated" });
                const imageConfig = NODE_DEFAULT_SIZE[CanvasNodeType.Image];
                const retryImage: CanvasNodeImage = {
                    ...
                    model: scheduled.target,
                };
                const generationMetadata = savedImageMetadata?.generationType
                    ? {
                          generationType: savedImageMetadata.generationType,
                          model: scheduled.target,
                          imageModelTargets: generationConfig.imageModelTargets,
                          size: generationConfig.size,
    ...
    const insertAssistantImage = useCallback(
        async (image: CanvasAssistantImage) => {
            const storedImage = image.storageKey ? { url: image.dataUrl, storageKey: image.storageKey, width: 1, height: 1, bytes: 0, mimeType: "image/png" } : await uploadImage(image.dataUrl, { origin: "generated" });
