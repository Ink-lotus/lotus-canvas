...
+ [修复] 局部重绘、多角度和插件生图三个入口纳入多渠道调度，不再绕过渠道并发上限，失败时可回退到其他已勾选渠道；蒙版不被支持的渠道会自动换渠道重试。
+ [修复] 桌面版改为运行时识别桌面环境，右上角版本号与更新检查不再依赖前端构建变量，避免漏设时静默退回上游 Web 版本号与上游更新源。
...
+ [新增] 增加 Windows 桌面端 NSIS/绿色版打包、校验和生成及带版本校验与签名钩子的 GitHub Actions 发布流程。
+ [新增] 桌面端改为仅检测更新并由用户确认下载与重启安装，区分绿色版 exe 同级 `data/` 和安装版 `%APPDATA%\lotus-canvas\data` 应用数据目录。

+ [新增] 偏好设置支持自定义图片反推提示词，反推图片节点时自动使用该默认内容。
+ [优化] 画布左侧资产卡片和 `@` 引用优先显示资产及文本节点标题，保留原有引用编号协议。
+ [新增] Electron 桌面版新增 `data/library` 普通媒体文件库、文件管理器定位，并在删除最后引用时移入系统回收站。
+ [调整] Electron 桌面版移除未发布的旧 IndexedDB 媒体迁移入口，保留新文件库和旧数据读取回退。
+ [调整] Electron 媒体文件库按 `external/generated`、媒体类型和日期分层保存文件。
+ [调整] 安装版和绿色版默认媒体库均使用各自 userData 下的 `library`，可在本地存储设置中迁移到规范化目录；卸载支持中英双语勾选删除应用数据。
+ [修复] 绿色版恢复媒体库位置设置，并与安装版共用目录选择、迁移和配置持久化逻辑。
+ [修复] 修复 NSIS 安装版卸载器未显示可选的应用数据删除勾选项。
+ [修复] 修正 NSIS 安装与卸载页面顺序，安装先选择用户范围再选择目录，卸载先显示欢迎页再选择组件。
+ [修复] 桌面版右上角版本号优先读取 Electron 应用版本，避免显示上游 Web 版本。
+ [调整] Windows 桌面版 GitHub Actions 正式发布暂不要求代码签名，并在 Release 说明中标注未签名状态。
+ [新增] 图片批量生成支持任务内选择多个模型名完全相同的渠道，并按各渠道并发上限共享调度请求。
+ [新增] 渠道模型支持设置统一模型名，允许不同渠道使用不同实际调用名时仍归入同一生图模型组。
+ [优化] 多渠道生图优先使用已勾选的默认生图模型渠道，单图失败时自动切换其他已勾选渠道。
+ [优化] 生图结果新增实际渠道角标，支持悬停显示、点击常驻，并在画布节点信息中展示渠道与模型。
+ [调整] 移除画布生成图片内容上覆盖显示的下载按钮，统一保持图片区域简洁。
+ [修复] 修复画布渠道多选弹层无法勾选、外部点击不关闭且误触下方节点的问题。
+ [修复] 画布新建生成配置和图片节点时沿用本画布最近一次勾选的生图渠道，刷新页面后回落到全局勾选。
+ [修复] 偏好设置切换默认生图模型时同步更新已勾选渠道，避免画布与图片工作台继续使用上一个模型的渠道。
