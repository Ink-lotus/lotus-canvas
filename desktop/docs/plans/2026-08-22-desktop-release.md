...
- `push` 到 `desktop-v*` tag：构建未签名发布产物并创建或更新 GitHub Release；代码签名暂缓。
...
- 未来启用签名时，证书通过 GitHub Secrets 注入，构建结束后删除临时证书文件；当前发布流程不注入证书。
...
## 阶段 3：Windows 代码签名（暂缓）

**目标：** 后续为公开发布的安装包和应用可执行文件增加可信发布者，降低 SmartScreen 和杀毒软件误报。当前阶段先发布未签名产物，并在 Release 说明中明确标注。
...
- [ ] 正式 Release 资产均有 SHA-256 校验值；当前版本在 Release 说明中明确标注未签名状态。
