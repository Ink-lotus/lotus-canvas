...
4. **预检是否会进入 `protocol.handle` 尚未证实。** 预检 OPTIONS 由网络栈生成而非渲染进程发起，是否路由到自定义协议处理器需实测确认。若不会进入，则预检仍走网络，由既有的 `onHeadersReceived`（404 → 200 改写）处理——该路径已验证可用，因此**此不确定性不构成回归风险**，仅意味着「本地应答预检」这一优化可能不生效
5. 前一设计的已知限制中，「中转站丢弃 OPTIONS 连接」一条在预检本地应答生效的前提下失效；若第 4 条实测为不生效，则该限制依然存在
